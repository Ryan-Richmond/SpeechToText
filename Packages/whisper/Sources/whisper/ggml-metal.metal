#define W_GGML_COMMON_DECL_METAL
#define W_GGML_COMMON_IMPL_METAL
...
template<w_ggml_sort_order order>
            ...
            if (ixj > col) {
                if ((col & k) == 0) {
                    if (dst_row[col] >= ncols ||
                        (dst_row[ixj] < ncols && (order == W_GGML_SORT_ORDER_ASC ?
                    ...
                    if (dst_row[ixj] >= ncols ||
                        (dst_row[col] < ncols && (order == W_GGML_SORT_ORDER_ASC ?
                            x_row[dst_row[col]] < x_row[dst_row[ixj]] :
                            x_row[dst_row[col]] > x_row[dst_row[ixj]]))
                    ) {
                        SWAP(dst_row[col], dst_row[ixj]);
...
template [[host_name("kernel_argsort_f32_i32_asc")]]  kernel argsort_t kernel_argsort_f32_i32<W_GGML_SORT_ORDER_ASC>;
template [[host_name("kernel_argsort_f32_i32_desc")]] kernel argsort_t kernel_argsort_f32_i32<W_GGML_SORT_ORDER_DESC>;
